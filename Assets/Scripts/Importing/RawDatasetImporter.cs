...
namespace UnityVolumeRendering
{
    ...
    public class RawDatasetImporter : DatasetImporterBase
    {
        ...
        public override VolumeDataset Import()
        {
            ...
            int uDimensionTemp = dimX * dimY * dimZ;
            int[] dataTemp = new int[uDimensionTemp];
            for (int i = 0; i < uDimensionTemp; i++)
            {
                dataTemp[i] = ReadDataValue(reader);
            }

            int uDimension = maxDim * maxDim * maxDim;
            ...
            for (int x = 0; x < maxDim; x++)
            {
                for (int y = 0; y < maxDim; y++)
                {
                    for (int z = 0; z < maxDim; z++)
                    {
                        ...
                        {
                            dataset.data[x + maxDim * y + maxDim * maxDim * z] = dataTemp[x + dimX * y + dimX * dimY * z];
                        }
                        else
                        {
                            dataset.data[x + maxDim * y + maxDim * maxDim * z] = 0;
                        }
                    }
                }
            }
