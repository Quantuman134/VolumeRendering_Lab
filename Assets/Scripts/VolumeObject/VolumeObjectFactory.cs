...
namespace UnityVolumeRendering
{
    public class VolumeObjectFactory
    {
        public static VolumeRenderedObject CreateObject(VolumeDataset dataset)
        {
            ...
            //gradient & normal
            Texture3D normTexture = dataset.GetNormTexture();
            Texture3D gradTexture = dataset.GetGradTexture();
            Texture3D gradMoldTexture = dataset.GetGradMoldTexture();
            //给shader传递纹理
            meshRenderer.sharedMaterial.SetTexture("_DataTex", dataset.GetDataTexture());
            meshRenderer.sharedMaterial.SetTexture("_NormTex", normTexture);
            meshRenderer.sharedMaterial.SetTexture("_GradTex", gradTexture);
            meshRenderer.sharedMaterial.SetTexture("_GradMoldTex", gradMoldTexture);
